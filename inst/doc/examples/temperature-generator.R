...
generation0 <- ComprehensiveTemperatureGenerator(station=vstation,Tx_all=TEMPERATURE_MAX,Tn_all=TEMPERATURE_MIN,year_min=1961,year_max=1990)
VAR_model <- generation0$var
CX <- generation0$input$monthly_mean_Tx
CN <- generation0$input$monthly_mean_Tn
generation1 <- ComprehensiveTemperatureGenerator(station=vstation,varmodel=generation0$var,
